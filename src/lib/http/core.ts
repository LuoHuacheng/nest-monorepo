...
type IRequest<T = any> = (url: string, opts?: IRequestOptions) => Promise<T>;

type IUpload<T = any, D = any> = (url: string, data: D, opts?: IRequestOptions) => Promise<T>;

type IErrorHandler = (error: RequestError, opts: IRequestOptions) => void;
...
class AxiosRequest {
	...
	static get instance() {
		...
		return AxiosRequest._instance ?? new AxiosRequest(singletonEnforcer);
	}
	 ...
	 */
	private mergeConfig() {
		this.config = merge(this.config, requestConfig);
	}
