...
interface TaroRouterOptions<S = SuccessCallback>
	extends
		Omit<Taro.navigateTo.Option, "success">,
...
function authCheck(urlKey: string, type: RouterType, options: TaroRouterOptions) {
	const isLogged = useAuthStore.getState().isAuthenticated;
	if (authRoutes.includes(urlKey)) {
		if (!isLogged) {
			...
			navigate("redirectTo", { ...options, url: "/pages/login/index" });
			return;
		}
...
class Router {
	...
	static get instance() {
		...
		return Router._instance ?? new Router(singletonEnforcer);
	}
		...
		try {
			if (type === "navigateBack") {
				...
				if (type === "switchTab") {
					...
					if (queryStr?.trim()) obj = searchParams2Obj(queryStr);

					const str = new URLSearchParams(merge(data as object, obj)).toString();
					url = str ? `${urlKey}?${str}` : urlKey;
				}
				authCheck(urlKey, type, { ...rest, events, url });
			}
		} catch (error) {
			// TODO
...
// 需要权限的路由,注意首尾不能带有斜杠
const authRoutes = ["pages/mine/index"];
// 全部路由
const routes = ["pages/index/index", "pages/match/index", "pages/pacer/index", "pages/mine/index"];
